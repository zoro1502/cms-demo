				...
				<Dropdown isOpen={ notificationsOpen } toggle={ () => toggleNotifications() } nav id="basic-nav-dropdown" className="ml-3">
					<DropdownToggle nav caret className="navbar-dropdown-toggle">
						<span style={ { border: '1px solid' } } className={ `${ s.avatar } rounded-circle float-left mr-2` }>
							<img src={ '/static/media/user.3ff01505.svg' } alt="User" />
						</span>
						<span className="small d-none d-sm-block ml-1 mr-2 body-1">{ 'admin@gmail.com' }</span>
					</DropdownToggle>
					<DropdownMenu className="navbar-dropdown profile-dropdown" style={ { width: "194px" } }>
						<NavLink href="#/template/profile">
							<DropdownItem className={ s.dropdownProfileItem }><ProfileIcon /><span>Trang cá nhân</span></DropdownItem>
						</NavLink>
						<NavItem>
							<NavLink onClick={ () => doLogout() } href="/login">
